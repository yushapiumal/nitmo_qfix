...
class _MobileCreateDocumentScreenState
  ...
  Future<void> _fetchSerialNumbers(int index, String itemModel,
      {int? subItemIndex}) async {
    print(
        'Fetching serial numbers for itemModel: $itemModel, index: $index, subItemIndex: $subItemIndex');
    ...
    try {
      final serials = await apiService.fetchSerialNumbers(itemModel);
      print('Fetched serial numbers: $serials');
      ...
      setState(() {
        if (subItemIndex != null && _selectedItems[index].subItems != null) {
          _selectedItems[index].subItems![subItemIndex].serialNumbers = serials;
        } else {
          _selectedItems[index].serialNumbers = serials;
        }
      });
    } catch (e) {
      ...
      Fluttertoast.showToast(
        msg: 'Failed to fetch serial numbers',
        toastLength: Toast.LENGTH_SHORT,
        gravity: ToastGravity.BOTTOM,
      );
...
class ItemsKitsPage extends StatefulWidget {
  ...
  final String? selectedPriceType;
  final String documentType;
  final Function(BuildContext, {required bool isKit, int? parentKitIndex}) onAddItemRow;
  final Function(BuildContext, SelectedItem, {int? parentKitIndex}) onEditItemRow;
  final Function(int, {int? parentKitIndex}) onRemoveItemRow;
  final Function(BuildContext, SelectedItem, int, {int? subItemIndex}) onSerialNumberTap;
  ...
  @override
  State<ItemsKitsPage> createState() => _ItemsKitsPageState();
}


class _ItemsKitsPageState extends State<ItemsKitsPage> {
      ...
      child: Column(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ] else if (widget.selectedGroup == 'item') ...[
                    Text(
                      'Select Items',
                      style: Theme.of(context).textTheme.titleLarge?.copyWith(
                            fontWeight: FontWeight.bold,
                  ...
                  const SizedBox(height: 16),
                  if (widget.availableItems.isEmpty && widget.availableKits.isEmpty && !widget.isLoading)
                    const Center(
                      child: Text(
                    ...
                    if (widget.selectedGroup == 'kit') ...[
                      ElevatedButton.icon(
                        ...
                        onPressed: widget.availableKits.isNotEmpty ? () => widget.onAddItemRow(context, isKit: true) : null,
                        style: ElevatedButton.styleFrom(
                      ...
                      ElevatedButton.icon(
                        ...
                        onPressed: widget.availableItems.isNotEmpty ? () => widget.onAddItemRow(context, isKit: false) : null,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: Colors.blue,
                          padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 24),
                        ),
          ...
          ListView.builder(
            ...
            itemCount: widget.selectedItems.where((item) => item.item != null && item.quantity > 0).length,
            itemBuilder: (context, index) {
              final validItems = widget.selectedItems
                  .asMap()
                  .entries
                  .where((entry) => entry.value.item != null && entry.value.quantity > 0)
                  .toList();
              final selectedItem = validItems[index].value;
              final originalIndex = validItems[index].key;
              final isKit = selectedItem.item is Kit;
              final kitItems = isKit ? (selectedItem.item as Kit).kitItems : null;
              return Card(
                elevation: 2,
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                InkWell(
                                  onTap: widget.documentType.toLowerCase() == 'invoice' &&
                                      ...
                                      ? () async {
                                          if (selectedItem.serialNumbers.isEmpty) {
                                            await widget.fetchSerialNumbers(originalIndex, selectedItem.item!.itemModel);
                                          }
                                          final updatedItem = widget.selectedItems[originalIndex];
                                          if (updatedItem.serialNumbers.isEmpty) {
                                            Fluttertoast.showToast(
                                              msg: 'This item is not in stock',
                                              toastLength: Toast.LENGTH_SHORT,
                                              gravity: ToastGravity.BOTTOM,
                                            ...
                                            widget.onSerialNumberTap(context, updatedItem, originalIndex);
                                          }
                                        }
                                      : null,
                                  child: Text(
                                    ...
                                    style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 16,
                                      decoration: widget.documentType.toLowerCase() == 'invoice' &&
                                              widget.selectedGroup != 'kit' &&
                                              selectedItem.item!.itemModel.isNotEmpty &&
                                              selectedItem.item!.itemModel != 'N/A'
                                ...
                                if (isKit) ...[
                                  const SizedBox(height: 4),
                                  Text(
                                    'Price (${widget.selectedPriceType ?? 'retail'}): Rs${selectedItem.getSelectedPrice(widget.selectedPriceType ?? 'retail').toStringAsFixed(2)}',
                                    style: const TextStyle(fontSize: 14),
                                  ),
                                  if (kitItems != null && kitItems.isNotEmpty) ...[
                                    const SizedBox(height: 8),
                                    const Text(
                                      'Items in Kit:',
                                      style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14),
                                    ),
                                    ...kitItems.entries.toList().asMap().entries.map((entry) {
                                      final subIndex = entry.key;
                                      final kitItem = entry.value.value;
                                      final kitItemSelected = selectedItem.subItems != null && subIndex < selectedItem.subItems!.length
                                                ...
                                                Navigator.push(
                                                  ...
                                                  MaterialPageRoute(
                                                    builder: (context) => AddItemPage(
                                                      ...
                                                      selectedGroup: 'item',
                                                      selectedPriceType: widget.selectedPriceType,
                                                      documentType: widget.documentType,
                                                      onItemSelected: (SelectedItem newItem) {
                                                        setState(() {
                                                          selectedItem.subItems ??= [];
                                                          if (subIndex < selectedItem.subItems!.length) {
                                                            selectedItem.subItems![subIndex] = newItem;
                                                          } else {
                                                      ...
                                                      onSerialNumberTap: (SelectedItem item) async {
                                                        ...
                                                        await widget.onSerialNumberTap(context, item, originalIndex, subItemIndex: subIndex);
                                                      },
                                                      fetchSerialNumbers: (String itemModel) async {
                                                        try {
                                                          await widget.fetchSerialNumbers(originalIndex, itemModel, subItemIndex: subIndex);
                                                          return selectedItem.subItems != null && subIndex < selectedItem.subItems!.length
                                                              ? selectedItem.subItems![subIndex].serialNumbers
                                                              : [];
                                                        } catch (e) {
                                                          return [];
                                                        }
                                                      },
                                                      initialItemName: kitItem.itemDescription,
                                                      initialItemModel: kitItem.itemModel, // Pass the item model here
                                                    ),
                                                  ),
                                                );
                                              },
                                  ...
                                  if (widget.documentType.toLowerCase() == 'invoice' &&
                                    ...
                                    InkWell(
                                      ...
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: selectedItem.selectedSerialNumbers.isNotEmpty
                                              ? Colors.yellow
                                ...
                                const SizedBox(height: 8),
                                Text(
                                  'Total Price: Rs${(selectedItem.getSelectedPrice(widget.selectedPriceType ?? 'retail') * selectedItem.quantity).toStringAsFixed(2)}',
                          ...
                          IconButton(
                            ...
                            onPressed: () => widget.onEditItemRow(context, selectedItem),
                          ),
                          IconButton(
                            icon: const Icon(Icons.delete, color: Colors.red),
                            tooltip: 'Remove ${isKit ? 'Kit' : 'Item'}',
                            onPressed: () {
                              print('Attempting to delete top-level item at originalIndex: $originalIndex at ${DateTime.now()}');
                              widget.onRemoveItemRow(originalIndex);
                            },
                          ),
                        ],
                      ),
                      if (isKit && widget.selectedGroup == 'kit') ...[
                        const SizedBox(height: 8),
                        Padding(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            onPressed: widget.availableItems.isNotEmpty
                                ? () => widget.onAddItemRow(context, isKit: false, parentKitIndex: originalIndex)
                                : null,
                            style: ElevatedButton.styleFrom(
                        ...
                        if (selectedItem.subItems != null && selectedItem.subItems!.isNotEmpty) ...[
                          const SizedBox(height: 8),
                          ListView.builder(
                            shrinkWrap: true,
                            physics: const NeverScrollableScrollPhysics(),
                                    ...
                                    child: Row(
                                      children: [
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: [
                                                  ...
                                                  Navigator.push(
                                                    ...
                                                    MaterialPageRoute(
                                                      builder: (context) => AddItemPage(
                                                        ...
                                                        onItemSelected: (SelectedItem editedItem) {
                                                          setState(() {
                                                            selectedItem.subItems![subIndex] = editedItem;
                                                          });
                                                          Navigator.pop(context);
                                                        },
                                                        onSerialNumberTap: (SelectedItem item) async {
                                                        ...
                                                        fetchSerialNumbers: (String itemModel) async {
                                                          try {
                                                            await widget.fetchSerialNumbers(originalIndex, itemModel, subItemIndex: subIndex);
                                                            return subItem.serialNumbers;
                                                          } catch (e) {
                                                            return [];
                                                          }
                                                        },
                                                        selectedItem: subItem,
                                                        initialItemName: subItem.item!.itemDescription,
                                                        initialItemModel: subItem.item!.itemModel, // Pass the item model here
                                                      ),
                                                    ),
                                                  );
                                                },
                                              ...
                                              const SizedBox(height: 4),
                                              Text(
                                                'Price (${widget.selectedPriceType ?? 'retail'}): Rs${subItem.getSelectedPrice(widget.selectedPriceType ?? 'retail').toStringAsFixed(2)}',
                                                style: const TextStyle(fontSize: 12),
                                              ),
                                              const SizedBox(height: 4),
                                              Text(
                                              ...
                                              const SizedBox(height: 4),
                                              Text(
                                                'Total Price: Rs${(subItem.getSelectedPrice(widget.selectedPriceType ?? 'retail') * subItem.quantity).toStringAsFixed(2)}',
                                                style: const TextStyle(
                                                  fontWeight: FontWeight.w600,
                                                  fontSize: 12,
                                                ),
                                        ...
                                        IconButton(
                                          ...
                                          tooltip: 'Edit Item',
                                          onPressed: () => widget.onEditItemRow(context, subItem, parentKitIndex: originalIndex),
                                        ),
                                        IconButton(
                                          icon: const Icon(Icons.delete, color: Colors.red, size: 20),
                                          tooltip: 'Remove Item',
                                          onPressed: () => widget.onRemoveItemRow(subIndex, parentKitIndex: originalIndex),
                                        ),
                                      ],
                                    ),
                                  ),
...
class _AddItemPageState extends State<AddItemPage> {
  ...
  void initState() {
    ...
    if (widget.selectedItem != null) {
      ...
      _newItem.quantityController.text = widget.selectedItem!.quantity.toString();
      // FIX: Fetch serial numbers if needed
      if (widget.documentType.toLowerCase() == 'invoice' &&
          widget.selectedGroup != 'kit' &&
          _newItem.item != null &&
          _newItem.item!.itemModel.isNotEmpty &&
          _newItem.item!.itemModel != 'N/A' &&
          _newItem.serialNumbers.isEmpty) {
        widget.fetchSerialNumbers(_newItem.item!.itemModel).then((serialNumbers) {
          setState(() {
      ...
      final initialItem = widget.availableItems.firstWhere(
        (item) => item.itemDescription == widget.initialItemName,
        orElse: () => widget.availableItems.isNotEmpty
            ? widget.availableItems.first
            : Item(
                id: '',
                itemName: '',
                itemDescription: '',
                itemQuantity: 0,
                itemModel: '',
                itemTag: '', itemRetailPrice: 0, itemsPeriodPrice: {},
              ),
      );
      if (initialItem.id.isNotEmpty) {
        _selectedItemId = initialItem.id;
        _newItem.item = initialItem;
        _newItem.quantity = 1;
  ...
  void _onItemChanged(String? newId) async {
    setState(() {
      if (newId != null) {
        final selected = widget.availableItems.firstWhere((item) => item.id == newId);
    ...
    if (newId != null &&
        ...
        _newItem.item!.itemModel.isNotEmpty &&
        _newItem.item!.itemModel != 'N/A') {
      final serialNumbers = await widget.fetchSerialNumbers(_newItem.item!.itemModel);
      setState(() {
        _newItem.serialNumbers = serialNumbers;
      });
    }
    ...
    setState(() {
      ...
      if (newQuantity != null && newQuantity > 0) {
        ...
        if (_newItem.selectedSerialNumbers.length > newQuantity) {
          _newItem.selectedSerialNumbers = _newItem.selectedSerialNumbers.sublist(0, newQuantity);
        }
      } else {
        _newItem.quantity = 0;
        _newItem.quantityController.text = '';
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    decoration: InputDecoration(
                      labelText: widget.selectedGroup == 'kit' ? 'Select Kit' : 'Select Item',
                    ...
                    items: widget.availableItems.map((Item item) {
                      return DropdownMenuItem<String>(
                        value: item.id,
                        child: Text(
                          widget.selectedGroup == 'kit' ? item.itemName : item.itemDescription,
                          overflow: TextOverflow.ellipsis,
                        ),
                      );
                    }).toList(),
                  ...
                  if (widget.selectedGroup != 'kit') ...[
                    ...
                    if (widget.documentType.toLowerCase() == 'invoice' &&
                      ...
                      const SizedBox(height: 16),
                      InkWell(
                        onTap: _newItem.serialNumbers.isNotEmpty ? _onScanTap : null,
                        child: Container(
                          padding: const EdgeInsets.all(8),
                          decoration: BoxDecoration(
                          ...
                          child: Text(
                            _newItem.selectedSerialNumbers.isEmpty
                                ? (_newItem.serialNumbers.isNotEmpty ? 'Tap to scan' : 'No S/N available')
                    ...
                    onPressed: () {
                      if (_formKey.currentState!.validate()) {
                        if (widget.documentType.toLowerCase() == 'invoice' &&
                            ...
                            _newItem.item!.itemModel.isNotEmpty &&
                            _newItem.item!.itemModel != 'N/A' &&
                            _newItem.selectedSerialNumbers.length != _newItem.quantity) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
